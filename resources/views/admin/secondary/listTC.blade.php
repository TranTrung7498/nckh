                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header card-header-success">
                                ...
                                <a href="{{url('admin/secondary/addTC')}}" class="btn btn-primary pull-right">Thêm mới</a>
                                <p class="card-category">Danh sách giảng viên trường Đại học Tài nguyên và Môi trường Hà Nội</p>
                            </div>
                            <div class="card-body">
                                <div class="col-md-12">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <input type="text" id="myInput" placeholder="Tìm kiếm tên giảng viên" title="Tìm kiếm tên giảng viên" class="form-control"><br>
                                        </div>
                                    </div>
                                </div>
                                <table class="table table-bordered" id="myTable">
                                    ...
                                    @foreach($giangvien as $gv)
                                        <tr>
                                            <td>{{$i++}}</td>
                                            <td>{{$gv->teacherName}}</td>
                                            <td>{{$gv->teacherCode}}</td>
                                            <td>{{$gv->phone}}</td>
                                            <td>{{$gv->email}}</td>
                                            <td>{{$gv->degree}}</td>
                                            <td>{{$gv->facultyName}}</td>
                                            <td>
                                                <a href="{{ url('admin/secondary/updateTC/'.$gv->id) }}" class="btn btn-warning btn-sm"
                                                   data-tr="{{$gv->id}}"
